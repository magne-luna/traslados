-- Migration: reparar_direcciones_lat_lng
-- Fix: `20260805140000_direcciones_geocoding.sql` quedó registrada como aplicada en
-- `supabase_migrations.schema_migrations` (probablemente por la colisión de nombre de archivo con
-- `20260805140000_fix_documentos_vehiculos_rls_modulo.sql`, mismo timestamp exacto, en un push
-- anterior) pero su contenido real NUNCA se ejecutó: las columnas `lat`/`lng` no existían en
-- `pacientes.direcciones` y `crear_paciente_completo` no tenía el paso de geocoding — confirmado
-- 2026-08-06 contra la base real (`information_schema.columns` y `pg_get_functiondef`) al
-- investigar el error PGRST204 "Could not find the 'lat' column of 'direcciones'".
--
-- Como esa versión ya figura aplicada, `supabase db push` nunca la vuelve a intentar — de ahí
-- que haga falta una migración nueva que repare el estado en vez de reintentar la original.
--
-- Agrega `lat`/`lng` (`IF NOT EXISTS` por si alguna quedó a medio aplicar) y reemplaza
-- `crear_paciente_completo` combinando AMBOS fixes que se pisaron entre sí: `descripcion`
-- (`20260806150000_direcciones_descripcion.sql`, si se aplicó completo) y `lat`/`lng` (el que
-- faltaba). Mismo `SECURITY INVOKER`/`search_path = ''` de siempre.
--
...
BEGIN
  ...
  --    sigue vigente). Persiste `descripcion` (texto libre opcional, NULLable) y `lat`/`lng`
  --    (geocoding, RF-701, NULLable) — los dos fixes que se habían pisado entre sí.
  FOR v_item IN
...
COMMENT ON FUNCTION pacientes.crear_paciente_completo(jsonb) IS
  ...
  '20260804120000, descripcion en direcciones desde 20260806150000 y lat/lng (geocoding, RF-701) '
  'desde 20260806160000 (reparación — 20260805140000 quedó registrada sin aplicarse). Ver '
  'openspec/changes/integracion-pacientes/design.md D4 y D9 addendum.';
