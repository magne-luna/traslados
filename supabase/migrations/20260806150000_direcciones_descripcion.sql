-- Migration: direcciones_descripcion
-- Change: pedido directo de la usuaria (2026-08-06), fuera del docx.
--
-- Agrega `descripcion` a `pacientes.direcciones`: texto libre opcional para diferenciar dos
-- direcciones del mismo `tipo_lugar` (ej. dos `terapia`: "Kinesióloga" vs "Fonoaudióloga"). El
-- dominio (`Direccion.descripcion`, shared/types/paciente.ts) lo modela opcional, y la columna
-- queda NULLable en la base por el mismo criterio — mismo tipo de discrepancia que `parentesco`
-- (`20260805130000_personas_a_cargo_parentesco.sql`, discrepancia de
-- knowledge-base/04_modelo_de_datos.md §Discrepancias).
--
-- ⚠️ El campo NO está en `docs/core/Traslados-Modelo-Datos.docx`. Documentado en
-- `knowledge-base/04_modelo_de_datos.md` §Discrepancias.
--
...
-- 1-7 letra por letra respecto de `20260805130000_personas_a_cargo_parentesco.sql` salvo el paso 4
-- (direcciones), que ahora también inserta `descripcion`.
--
...
ALTER TABLE pacientes.direcciones ADD COLUMN descripcion TEXT;
...
BEGIN
  ...
  --    sigue vigente).
  --    ⚠️ FIX de esta migración: ahora también persiste `descripcion` (texto libre opcional,
  --    NULLable — ver cabecera).
  FOR v_item IN
    SELECT value FROM jsonb_array_elements(COALESCE(p_paciente -> 'direcciones', '[]'::jsonb))
  LOOP
    INSERT INTO pacientes.direcciones (paciente_id, calle, numero, tipo_lugar, localidad, descripcion)
    VALUES (
      v_paciente_id,
      v_item ->> 'calle',
      NULL,
...
COMMENT ON FUNCTION pacientes.crear_paciente_completo(jsonb) IS
  ...
  '20260804120000 y descripcion en direcciones desde 20260806150000. Ver '
