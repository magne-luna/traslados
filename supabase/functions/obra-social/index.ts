...
function toDb(input: ObraSocialInput): Record<string, unknown> {
  ...
  if (input.tipoComprobante !== undefined)
    row.tipo_comprobante = input.tipoComprobante;
  if (input.plazoCobroDias !== undefined)
    row.plazo_cobro_dias = input.plazoCobroDias;
  if (input.modalidadFacturacion !== undefined)
    row.modalidad_facturacion = input.modalidadFacturacion;
  if (input.admitePagosParciales !== undefined)
    row.admite_pagos_parciales = input.admitePagosParciales;
  if (input.identificadorOrigen !== undefined)
    row.identificador_origen = input.identificadorOrigen;
  return row;
}
