...
describe('supabasePacienteRepository.update — diff de colecciones (3.9)', () => {
  it('editar una dirección la actualiza por id, sin borrarla ni reinsertarla', async () => {
    ...
    expect(upserts[0]?.payload).toEqual([
      { id: 'd-1', calle: 'San Martín 999', numero: null, tipo_lugar: 'domicilio', localidad: '', descripcion: null, paciente_id: 'p-1' },
  ...
  it('inserta las direcciones nuevas y borra las que ya no están', async () => {
    ...
    expect(upserts).toHaveLength(1);
    expect(upserts[0]?.payload).toEqual([
      { id: 'd-1', calle: 'San Martín', numero: null, tipo_lugar: 'domicilio', localidad: '', descripcion: null, paciente_id: 'p-1' },
      { id: 'd-3', calle: 'Nueva 456', numero: null, tipo_lugar: 'terapia', localidad: '', descripcion: null, paciente_id: 'p-1' },
    ]);
