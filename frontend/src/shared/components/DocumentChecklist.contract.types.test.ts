...
// componente compartido. Este archivo deja constancia explícita de que el contrato de props de
// `DocumentChecklist` sigue siendo EXACTAMENTE el mismo que antes de este change — ningún prop
// `agrupacion`/`grupos` nuevo, ninguno de los 7 props existentes cambió de tipo/opcionalidad.
//
...
// señal que realmente se rompe si el contrato cambia es comparar el tipo completo a nivel de tipo,
// no un render puntual (que es justo lo que ya cubre extensamente `DocumentChecklist.test.tsx`,
// pero solo a nivel de comportamiento, no de forma exacta del tipo de sus props).

type Equal<X, Y> = (<T>() => T extends X ? 1 : 2) extends <T>() => T extends Y ? 1 : 2 ? true : false;
type Expect<T extends true> = T;

// El contrato original, tal como estaba antes de `documentos-checklist-por-actividad` (heredado de
...
// Si `DocumentChecklist` hubiera ganado un prop `agrupacion`/`grupos` (la rama "opción C" que este
// change explícitamente NO tomó) o hubiera perdido/mutado alguno de los 7 existentes, esta línea
// deja de compilar — es la señal dura de regresión de contrato.
