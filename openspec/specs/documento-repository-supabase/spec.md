# Documento Repository Supabase

## Purpose

Define la implementación real de `DocumentoRepository` contra Supabase Storage y Postgres
(`SupabaseDocumentoRepository.ts` + `documentoMapping.ts`), entregada por `integracion-documentos`
para el módulo de documentos de las 4 entidades (Pacientes, Vehículos, Conductores, Facturas). Cubre
la configuración declarativa por entidad, el mapeo puro fila↔dominio, el orden de operaciones que
compensa la falta de transacción compartida entre Storage y Postgres, la traducción de errores, y el
alcance real del swap del composition root (solo Pacientes conectado; Vehículos/Conductores/Facturas
siguen en `mockDocumentoRepository` hasta que sus propios changes de integración aterricen).
...
- **WHEN** se comparan con el schema verificado en vivo (`tasks.md` 1.2/1.3 de `integracion-documentos`)
...
`toInsertPayload`, `inferirTipoMime`), sin efectos, sin reloj global, sin red y **sin generar el UUID
internamente** — el UUID SHALL entrar por parámetro. Las funciones de parseo MUST angostar `unknown`
con type guards explícitos y MUST descartar —no propagar— una fila malformada sin romper la
colección.

Ninguna de las 4 tablas reales persiste `vigenciaDesde` ni `tipoMime`: `vigenciaDesde` queda
`undefined` (válido, campo opcional del tipo). `tipoMime` en cambio SHALL derivarse de la extensión
de `nombreArchivo` (`inferirTipoMime`) en vez de quedar `undefined` a secas — sin esa derivación, la
acción "Ver" del checklist no puede decidir cómo previsualizar un documento real (hallazgo de
verificación manual, `integracion-documentos` `tasks.md` §8.3).
...
#### Scenario: El tipo MIME se infiere de la extensión, nunca se inventa
- **GIVEN** un `nombreArchivo` con extensión `.pdf`, `.png`, `.jpg`, `.jpeg`, `.gif` o `.webp`
- **WHEN** se invoca `inferirTipoMime`
- **THEN** devuelve el tipo MIME correspondiente, sin distinguir mayúsculas/minúsculas
- **AND** para cualquier otra extensión o nombre sin extensión reconocible devuelve `undefined`
...
`columnaEntidad`, y MUST NOT invocar la API de Storage: los campos que la UI necesita (`itemId`,
`nombreArchivo`, `subidoEn`, `tipoMime`) se resuelven a partir de la fila.
...
comentario, hasta que `integracion-conductores-vehiculos` e `integracion-facturacion` aterricen.
`mockDocumentoRepository` MUST NOT eliminarse.
