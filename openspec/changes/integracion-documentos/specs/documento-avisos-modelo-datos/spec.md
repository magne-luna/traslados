...
rechazaría (`design.md` Checkpoint 0).
...
`FacturaDocumentos.tsx` muestra hoy un `AvisoModeloDatos` que dice *"El docx no modela ninguna tabla
de documentos por Factura (Discrepancia 2). El backend `C-07` debe crear `documento_factura`"*.
**Esa tabla ya existe**: la creó `C-03` (`20260729100000_schema_documento_factura.sql`) y está
verificada en vivo, con RLS y trigger de auditoría.

El sistema SHALL reemplazar ese texto por el estado real: la discrepancia contra el docx **sigue
vigente** (el docx no modela documentos por factura), pero la tabla ya no falta — lo que falta es que
`Factura` deje de ser mock. MUST NOT borrarse la mención a la discrepancia con el docx: sigue siendo
cierta y sigue pendiente de confirmar con su dueño.

#### Scenario: El aviso ya no pide una tabla que existe
- **GIVEN** `FacturaDocumentos.tsx` después de este change
...
Toda discrepancia descubierta por este change SHALL documentarse en los dos lugares que exige
`CLAUDE.md` —`knowledge-base/04_modelo_de_datos.md` §Discrepancias y el bullet correspondiente de
`CHANGES.md`— y, cuando aplique a una pantalla, con `AvisoModeloDatos`. MUST NOT resolverse
unilateralmente en el código.

Las discrepancias a documentar en este change son:
1. `archivo_url` (4 tablas) **no guarda una URL**, guarda la clave del objeto dentro del bucket.
2. El bucket `documentos-vehiculos` está gateado por el módulo `conductores` mientras su tabla
   `conductores.documentacion_vehiculo` pide `vehiculos` (desfasaje introducido por
   `20260730140000_split_modulos_permisos.sql`, que no tocó `storage.objects`).
...
4. `conductores.documentacion_conductores` es la única de las 4 tablas **sin `created_at`**, un gap
   de trazabilidad (RN-GL-02) que existía desde `20260724100006_schema_conductores.sql` y que nadie
   había notado porque la tabla nunca se usó.
...
- **GIVEN** el texto de `CHANGES.md` §C-03, que dice *"documentos-vehiculos cae bajo conductores, no
