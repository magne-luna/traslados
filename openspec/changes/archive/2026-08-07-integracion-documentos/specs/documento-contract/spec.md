...
Hasta este change, `DocumentoRepository` tenía **una sola** implementación
...
`frontend/src/shared/types/documento.ts` MUST NOT modificarse en este change: es un reemplazo de la
capa de datos, no un cambio de contrato.

#### Scenario: El mock sigue existiendo y sigue siendo válido
- **GIVEN** `mockDocumentoRepository`
- **WHEN** este change se completa
...
MUST conservar sus firmas y su forma. El único cambio admitido en los wrappers es el agregado de un
`AvisoModeloDatos` (ver `documento-avisos-modelo-datos`).
...
- **AND** este change no generó ninguna URL pública ni firmada
