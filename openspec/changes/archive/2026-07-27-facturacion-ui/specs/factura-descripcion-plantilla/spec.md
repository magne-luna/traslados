...
### Requirement: Descripción persistida y no recalculada
El sistema SHALL persistir la descripción renderizada en `Factura.descripcion` (campo que el docx sí modela) y MUST NOT recalcularla al mostrar o imprimir una factura ya emitida, para respetar RN-FA-06 (los ajustes de valores de facturas emitidas no son retroactivos).

#### Scenario: El detalle y la vista imprimible leen la descripción persistida
- **WHEN** se abre el detalle o la vista imprimible de una factura emitida
- **THEN** se muestra la `descripcion` guardada, sin volver a invocar `renderDescripcionFactura`

#### Scenario: Vista previa antes de emitir
- **WHEN** el usuario está cargando una factura todavía en estado `a-facturar`
- **THEN** la pantalla muestra una vista previa de la descripción renderizada en vivo, que se congela recién al pasar a `facturado`
