...
MUST NOT ser ejecutable por el rol `anon`.
...
### Requirement: Número de afiliado desde la cobertura, con degradación explícita

El sistema SHALL leer `Paciente.numeroAfiliado.valor` desde
`obra_social.coberturas_paciente.num_afiliado`, tomando la cobertura del paciente cuyo
`obra_social_id` coincide con el del paciente y cuya `fecha_desde` es la más reciente. Dado que esa
tabla pertenece al schema `obra_social` y está gateada por el permiso del módulo `obra_social`, el
sistema MUST degradar a valor vacío —sin lanzar— cuando la consulta falle o no devuelva filas, y
MUST NOT impedir que se muestre el resto de la ficha. El sistema SHALL escribir sobre
`coberturas_paciente` únicamente cuando `numeroAfiliado.valor` cambió, y SHALL traducir un rechazo
de RLS de esa escritura a un mensaje que nombre el módulo Obras Sociales.
...
#### Scenario: El formato del identificador no se persiste

- **GIVEN** que no existe columna para el formato del identificador de afiliado (IN-01 abierta)
- **WHEN** se lee un paciente
- **THEN** `numeroAfiliado.formato` toma el default documentado (`DEFAULT_FORMATO_AFILIADO`) y queda
  editable en el formulario
- **AND** al guardar, el formato NO se envía a la base
- **AND** la pantalla señala con `AvisoModeloDatos` que ese dato no se guarda
...
- **GIVEN** los campos `Direccion.localidad`, `Direccion.dias`, `Direccion.horario`,
  `Paciente.amparoJudicialAclaracion` y `IdentificadorAfiliado.formato`, que no tienen columna
