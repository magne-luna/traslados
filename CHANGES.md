...
- **Progreso frontend (mock, vía FE-0)**: ✅ pantalla de login mock y guard de rutas protegidas (`RequireAuth`), sin roles ni matriz Cuenta→Módulos todavía (`useAuth()` solo modela sesión sí/no, a propósito — ver design.md del change archivado). **Falta**: Supabase Auth real, tabla `permisos_modulo`, RLS, `audit_log`, pantalla de gestión de cuentas.
- **⚠️ Hack temporal solo para demo (2026-07-27, fuera del flujo OPSX)**: `LoginPage.tsx` tiene campos de email/contraseña precargados con datos falsos (`andrea@traslados.mock` / `demo1234`) — 100% cosmético, `handleSubmit` no los lee ni valida. `AuthContext.tsx` arranca deslogueado solo si `VITE_DEMO_MODE=true` (env var, no versionada); sin esa variable el dev sigue arrancando logueado como siempre. **Al implementar C-02**: borrar `DEMO_EMAIL`/`DEMO_PASSWORD` y los `defaultValue` de `LoginPage.tsx`, y sacar el chequeo de `VITE_DEMO_MODE` de `AuthContext.tsx` (con Supabase Auth real, la sesión ya no arranca "logueada por defecto" en ningún escenario).
- **⚠️ Discrepancia con `docs/core/Traslados-Modelo-Datos.docx`**: el docx describe un campo `Rol` fijo en Usuarios (Administrador con bypass total / Empleado con permisos por módulo) más una regla de "protección contra autopromoción de rol" — contradice el "sin roles fijos" de `03_actores_y_roles.md`. Resolver con quien armó el docx **antes** de diseñar las RLS de este change (ver nota agregada en `03_actores_y_roles.md`).
